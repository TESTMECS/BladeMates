...
import { HomeIcon, UserIcon, TrendingUpIcon } from "@heroicons/react/outline";
import FeedType from "../types/Feed";
interface NavbarProps {
  setFeed: (feed: FeedType) => void;
}

const Navbar: React.FC<NavbarProps> = ({ setFeed }) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav className="flex flex-col h-16 space-y-4">
          <div className="flex flex-col space-y-4">
            <Link
              to="/home"
              className="flex items-center hover:text-indigo-500 px-3 py-2 rounded-md text-lg font-semibold transition-colors duration-200"
            >
              LOGO HERE
            </Link>
